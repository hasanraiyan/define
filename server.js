...
// --- Metrics Store ---
const metrics = {
    totalRequests: 0, // Total requests reaching handleDefineRequest
    requestsSucceededExact: 0, // API requests where exact word count was achieved
    requestsSucceededBestEffort: 0, // API requests where best effort result was returned
    requestsFailedInputValidation: 0,
    requestsFailedApiErrors: 0, // Failed after retries exhausted due to API issues
    requestsFailedServerErrors: 0, // Failed due to unexpected internal server errors
    apiCallErrors: 0, // Total count of *individual* API call errors across all attempts
    cacheHits: 0, // Requests successfully served directly from DB
    cacheMisses: 0, // Requests that required calling the API after DB lookup
    cacheWritesSuccess: 0, // Successful cache writes
    cacheWritesFailed: 0, // Failed cache writes (e.g., duplicate key)
...
async function handleDefineRequest(params, req, res) {
    ...
    try {
        ...
        const attemptHistorySummary = apiResult.attemptHistory.map(a => ({
         ...
         const minTemp = apiResult.attemptHistory.reduce((min, a) => Math.min(min, a.temperature), INITIAL_TEMPERATURE);
         const maxTemp = apiResult.attemptHistory.reduce((max, a) => Math.max(max, a.temperature), INITIAL_TEMPERATURE);
        ...
        // This catch block handles errors not caught within the retry loop (e.g., logic errors)
...
app.get('/api/metrics', (req, res) => {
    ...
    const totalProcessedRequests = totalCompletedRequests + totalFailedRequests; // Total requests that reached handleDefineRequest after initial middleware

    // Calculate success rates based on total requests *processed* by the handler
    const successRateExactApi = totalProcessedRequests > 0 ? (metrics.requestsSucceededExact / totalProcessedRequests * 100) : 0;
    ...
    res.json({
        ...
        totalRequestsReceived: metrics.totalRequests, // Total requests reaching handler
...
process.on('uncaughtException', (error) => {
    ...
    logger.on('finish', () => { // Ensure logs are flushed before exiting
        process.exit(1);
    });
     // Also set a timeout in case logging fails
    setTimeout(() => {
      process.exit(1);
    }, 500); // Give logger 500ms to flush
});
...
async function gracefulShutdown(signal) {
    ...
    setTimeout(() => {
        ...
        process.exit(1);
    }, 20000); // Increased timeout slightly (20 seconds)
}